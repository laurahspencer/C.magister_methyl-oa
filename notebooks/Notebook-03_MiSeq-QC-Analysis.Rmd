...
summary2.0 <- read_delim(file="../qc-processing/MiSeq-QC/mbd_key.txt", delim="\t") %>% clean_names() %>%
  right_join(
    ...
    by=c("sample"="File")) %>%
  rename(Total_Reads=`Total Reads`, Aligned_Reads=`Aligned Reads`, Unaligned_Reads=`Unaligned Reads`, Ambiguously_Aligned_Reads=`Ambiguously Aligned Reads`, 
         Unique_reads=`Unique Reads (remaining)`, CpGs_Meth=`Methylated CpGs`, CHGs_Meth=`Methylated chgs`, CHHs_Meth=`Methylated CHHs`, Low_CpGmeth=low_cp_gmeth) %>%
